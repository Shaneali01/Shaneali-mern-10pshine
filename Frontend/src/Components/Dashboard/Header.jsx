...
const Header = ({ handleCreateNote,handleLogout }) => { // Removed handleLogout prop—handle locally
  const [isProfileDropdownOpen, setIsProfileDropdownOpen] = useState(false);
  
  const user = JSON.parse(localStorage.getItem('user')) || {};
  const userImage = user.image;
  
  const navigate = useNavigate(); // For local navigation (e.g., to profile and logout)

  const handleEditProfile = () => {
    navigate('/profile'); // Actual navigation to profile page
    setIsProfileDropdownOpen(false);
  };

  // Self-contained logout logic—no prop needed
